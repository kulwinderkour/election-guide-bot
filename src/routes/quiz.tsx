  ...
  head: () => ({
    meta: [
      { title: "Election Quiz — ElectionGuide Bot" },
      {
        name: "description",
        content:
          "Test your knowledge of Indian elections with a 6-question interactive quiz. Get explanations and earn a civic-knowledge badge.",
      },
      { property: "og:title", content: "Take the Indian Elections Quiz" },
      {
        property: "og:description",
        content: "Six quick questions to test your civic knowledge. Earn a badge!",
      },
    ],
  }),
});
...
function QuizPage() {
  ...
  useEffect(() => {
    googleAnalytics.trackQuizStart("General", "Medium");
  }, []);

  useEffect(() => {
    if (done) {
      googleAnalytics.trackQuizComplete("General", score, QUIZ.length);
    }
  }, [done, score]);
      ...
      <main className="mx-auto max-w-2xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        {!done ? (
          <>
            ...
            <div
              key={idx}
              className="mt-6 animate-fade-up rounded-2xl border border-border bg-gradient-card p-6 shadow-soft sm:p-8"
            >
                ...
                {q.options.map((opt, i) => {
                  ...
                  if (picked !== null) {
                    if (isAnswer) style = "border-india-green bg-india-green/10 text-foreground";
                      ...
                      style = "border-destructive bg-destructive/10 text-foreground";
                  ...
                  return (
                    ...
                    >
                      ...
                      {picked !== null && isAnswer && (
                        <CheckCircle2 className="h-5 w-5 text-india-green shrink-0" />
                      )}
          ...
          <div className="mt-8 animate-fade-up overflow-hidden rounded-2xl border border-border shadow-elegant">
            <div className="bg-gradient-hero p-8 text-center text-primary-foreground">
              ...
              <div className="mt-4 inline-flex rounded-full bg-white/15 px-4 py-1.5 text-sm font-semibold backdrop-blur">
                {badge}
              </div>
            </div>
            <div className="space-y-3 bg-card p-6 text-center">
              <p className="text-sm text-muted-foreground">
                {score === QUIZ.length
                  ? "Perfect run! You're ready to teach others."
                  : "Great effort. Brush up with the chat or timeline."}
              </p>
              <div className="flex flex-col gap-2 sm:flex-row">
                <button
                  onClick={reset}
                  className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-secondary"
                >
                  <RotateCcw className="h-4 w-4" /> Try again
                </button>
                <Link
                  to="/chat"
                  className="inline-flex flex-1 items-center justify-center rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90"
                >
                  Ask the bot →
                </Link>
              </div>
